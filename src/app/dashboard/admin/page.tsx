  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#0D241C]/5">
          <div className="w-12 h-12 bg-[#E5EDDE] rounded-full flex items-center justify-center mb-4">
            <Activity className="text-[#071a12]" />
          </div>
          <p className="text-[#0D241C]/60 font-medium mb-1">Procedures Billed</p>
          {loading ? <Skeleton className="h-9 w-16 mt-1" /> : <h2 className="text-3xl font-bold">{appointments.length * 2}</h2>}
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#0D241C]/5">
          <div className="w-12 h-12 bg-red-50 rounded-full flex items-center justify-center mb-4">
            <DollarSign className="text-red-500" />
          </div>
          <p className="text-[#0D241C]/60 font-medium mb-1">Outstanding</p>
          {loading ? <Skeleton className="h-9 w-16 mt-1" /> : <h2 className="text-3xl font-bold text-red-600">$4,250</h2>}
        </div>
        <div className="bg-[#071a12] text-white p-6 rounded-2xl shadow-sm border border-[#0D241C]/5 md:col-span-2 flex justify-between items-center relative overflow-hidden">
          <div className="relative z-10">
            <p className="text-white/70 font-medium mb-1">Monthly Revenue Collected</p>
            <h2 className="text-4xl font-bold text-[#c9a973]">$42,500.00</h2>
            <p className="text-sm text-white/50 mt-2">Daily Pending: $1,200.00</p>
            <button 
              onClick={() => {
              ...
              className="mt-4 px-4 py-2 bg-[#c9a973]/20 text-[#c9a973] hover:bg-[#c9a973] hover:text-[#071a12] rounded-lg text-sm font-semibold transition-colors flex items-center gap-2"
            >
              <Download size={16} /> Export CSV
            </button>
          </div>
          <DollarSign size={120} className="text-white/5 absolute right-0 bottom-0 translate-x-1/4 translate-y-1/4" />
        </div>
      </div>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              appointments.map(apt => (
                <div key={apt.id} className="p-4 rounded-xl border border-[#0D241C]/5 bg-[#E5EDDE]/20 flex justify-between items-center">
                  <div>
