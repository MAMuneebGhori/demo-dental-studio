  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-[#0D241C]/60 text-lg">Manage your patients and clinical notes for today.</p>
      </div>
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
            <div>
              ...
              <input type="time" value={shiftStart} onChange={e => setShiftStart(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-2 focus:outline-none focus:border-[#c9a973]" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-white/70 mb-2">End Time</label>
              <input type="time" value={shiftEnd} onChange={e => setShiftEnd(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-2 focus:outline-none focus:border-[#c9a973]" />
            </div>
            <button 
              onClick={handleSaveShift}
              disabled={savingShift}
              className="w-full py-2 bg-[#c9a973] text-[#071a12] font-bold rounded-lg hover:bg-[#b09363] transition-colors disabled:opacity-50 flex justify-center items-center gap-2"
            >
        ...
        <div className="divide-y divide-[#0D241C]/5">
          {loading ? (
            ...
            upcomingAppointments.map((apt) => (
              <div key={apt.id} className="p-6 hover:bg-[#E5EDDE]/10 transition-colors flex flex-col md:flex-row justify-between items-start md:items-center gap-4 group">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-[#071a12] text-[#c9a973] flex items-center justify-center font-bold font-serif text-xl shadow-inner">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                  ...
                  <Link href={`/dashboard/doctor/patient/${apt.id}`} className="ml-auto md:ml-4 px-4 py-2 bg-[#071a12] text-[#c9a973] rounded-lg text-sm font-semibold hover:bg-[#0f2e21] transition-colors flex items-center gap-2 group-hover:shadow-md">
